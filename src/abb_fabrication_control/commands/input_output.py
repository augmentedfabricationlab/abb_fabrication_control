...
def open_gripper(robot, send_and_wait=False):
    ...
    """
    return set_digital_out(robot, 'Ausgang_100_0', output_state=0, send_and_wait=send_and_wait)

def close_gripper(robot, send_and_wait=False):
    """ Send signal to open the gripper
    """
    return set_digital_out(robot, 'Ausgang_100_0', output_state=1, send_and_wait=send_and_wait)
